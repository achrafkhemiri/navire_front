


export interface UserData {
  id: string;
  name: string;
  progress: string;
  fruit: string;
}

import { AfterViewInit, Component, ViewChild, ViewEncapsulation } from '@angular/core';
import { MatDialog } from '@angular/material/dialog';
import { EditRowDialogComponent } from './edit-row-dialog.component';
import { MatPaginator } from '@angular/material/paginator';
import { MatSort } from '@angular/material/sort';
import { MatTableDataSource } from '@angular/material/table';

const FRUITS: string[] = [
  'blueberry', 'lychee', 'kiwi', 'mango', 'peach', 'lime', 'pomegranate', 'pineapple',
];
const NAMES: string[] = [
  'Maia', 'Asher', 'Olivia', 'Atticus', 'Amelia', 'Jack', 'Charlotte', 'Theodore',
  'Isla', 'Oliver', 'Isabella', 'Jasper', 'Cora', 'Levi', 'Violet', 'Arthur', 'Mia', 'Thomas', 'Elizabeth',
];

function createNewUser(id: number): UserData {
  const name =
    NAMES[Math.round(Math.random() * (NAMES.length - 1))] +
    ' ' +
    NAMES[Math.round(Math.random() * (NAMES.length - 1))].charAt(0) +
    '.';
  return {
    id: id.toString(),
    name: name,
    progress: Math.round(Math.random() * 100).toString(),
    fruit: FRUITS[Math.round(Math.random() * (FRUITS.length - 1))],
  };
}

@Component({
  selector: 'app-etat-generale',
  templateUrl: './etat-generale.component.html',
  styleUrls: ['./etat-generale.component.css'],
    encapsulation: ViewEncapsulation.None   // 🔥 important
})
export class EtatGeneraleComponent implements AfterViewInit {
  isSidebarOpen = true;
  displayedColumns: string[] = ['id', 'name', 'progress', 'fruit', 'actions'];
  dataSource: MatTableDataSource<UserData>;

  @ViewChild(MatPaginator) paginator!: MatPaginator;
  @ViewChild(MatSort) sort!: MatSort;

  constructor(private dialog: MatDialog) {
    const users = Array.from({ length: 13 }, (_, k) => createNewUser(k + 1));
    this.dataSource = new MatTableDataSource(users);
  }

  ngAfterViewInit() {
    this.dataSource.paginator = this.paginator;
    this.dataSource.sort = this.sort;
  }

  applyFilter(event: Event) {
    const filterValue = (event.target as HTMLInputElement).value;
    this.dataSource.filter = filterValue.trim().toLowerCase();
    if (this.dataSource.paginator) {
      this.dataSource.paginator.firstPage();
    }
  }

  addRow() {
    const data = this.dataSource.data;
    const newRow: UserData = {
      id: (data.length + 1).toString(),
      name: '',
      progress: '0',
      fruit: ''
    };
    this.dataSource.data = [...data, newRow];
  }

    editRow(row: UserData) {
      this.openEditDialog(row);
    }

    openEditDialog(row: UserData) {
      const dialogRef = this.dialog.open(EditRowDialogComponent, {
        width: '400px',
        data: { ...row }
      });
      dialogRef.afterClosed().subscribe(result => {
        if (result) {
          row.name = result.name;
          row.progress = result.progress;
          row.fruit = result.fruit;
          this.dataSource.data = [...this.dataSource.data];
        }
      });
    }

    deleteRow(row: UserData) {
      this.dataSource.data = this.dataSource.data.filter(r => r !== row);
    }
}
